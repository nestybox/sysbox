...
In addition, the sysbox-deploy-k8s creates a couple of ephemeral systemd
services on the K8s nodes where Sysbox is installed. These ephemeral systemd
services are short-lived and help with the installation of CRI-O and
Sysbox. They are caled `crio-installer` and `sysbox-installer-helper`. Look at
the logs generated by these systemd services in the K8s worker nodes to make sure
all is good:
...
If for some reason one of the pods remains in an error state forever, please
open an issue in the Sysbox repo so we can investigate. As a work-around, if the
pod is part of a deployment or daemonset, removing the pod may fix the issue as
it causes Kubernetes to re-create it.
