...
@test "mount procfs" {
  ...
  docker exec "$syscont" bash -c "cat $mnt_path/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  ...
  docker exec "$syscont" bash -c "echo $nf_max_proc > /proc/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  ...
  nf_max_proc=$((nf_max_proc - 100))
  docker exec "$syscont" bash -c "echo $nf_max_proc > $mnt_path/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
...
@test "mount non-procfs" {
  ...
  docker exec "$syscont" bash -c "mkdir /root/sys"
  [ "$status" -eq 0 ]

  docker exec "$syscont" bash -c "mount -t sysfs sysfs /root/sys"
  [ "$status" -eq 0 ]

  docker exec "$syscont" bash -c "mount | grep /root/sys"
  [ "$status" -eq 0 ]
...
@test "multiple procfs in one syscont" {
  ...
  docker exec "$syscont" bash -c "cat $mnt_path1/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  local nf_max_path1="$output"

  docker exec "$syscont" bash -c "cat $mnt_path2/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  ...
  docker exec "$syscont" bash -c "echo $nf_max_path1 > $mnt_path1/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
...
@test "multiple procfs in multiple syscont" {
  ...
  docker exec "$syscont0" bash -c "cat $mnt_path/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  local nf_max0="$output"

  nf_max0=$((nf_max0 - 100))

  docker exec "$syscont0" bash -c "echo $nf_max0 > $mnt_path/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]

  docker exec "$syscont0" bash -c "cat /proc/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  [ $output -eq $nf_max0 ]
  ...
  docker exec "$syscont1" bash -c "cat /proc/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  [ $output -ne $nf_max0 ]

  docker exec "$syscont1" bash -c "cat $mnt_path/sys/net/netfilter/nf_conntrack_max"
  [ "$status" -eq 0 ]
  [ $output -ne $nf_max0 ]
...
@test "bind mount ignore" {
  ...
  docker exec "$syscont" bash -c "capsh --inh=\"\" --drop=cap_sys_admin -- -c \"mount --bind $mnt_path/sys $mnt_path/sys\""
  [ "$status" -eq 1 ]
  [[ "$output" =~ "permission denied" ]]

  # root user without CAP_DAC_OVERRIDE, CAP_DAC_READ_SEARCH can't bind-mount if path is non-searchable
  docker exec "$syscont" bash -c "chmod 666 /root/l1"
  docker exec "$syscont" bash -c "capsh --inh=\"\" --drop=cap_dac_override,cap_dac_read_search -- -c \"mount --bind $mnt_path/sys $mnt_path/sys\""
  [ "$status" -ne 0 ]
  [[ "$output" =~ "Permission denied" ]]
...
@test "procfs rbind partial unmount" {
  ...
  docker exec -d "$syscont" bash -c "mount --rbind / $mnt_path"
  [ "$status" -eq 0 ]
  verify_syscont_procfs_mnt $syscont $mnt_path/proc
  #verify_syscont_sysfs_mnt $syscont $mnt_path2/sys

  # Unmount procfs sysbox-fs backed submounts. As we are in systemd's path, sysbox-fs
  # should honor this request.
  for node in "${PROCFS_EMU[@]}"; do
    docker exec "$syscont" bash -c "umount $mnt_path/proc/$node"
    [ "$status" -eq 0 ]
    docker exec "$syscont" bash -c "mount | egrep \"$mnt_path/proc/$node \""
    [ "$status" -eq 1 ]
  done
