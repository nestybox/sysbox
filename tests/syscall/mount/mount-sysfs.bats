...
@test "mount sysfs read-only" {
  ...
  docker exec "$syscont" bash -c "echo 0 > $mnt_path/module/nf_conntrack/parameters/hashsize"
  [ "$status" -eq 1 ]
...
@test "sysfs consistency (multiple procfs in multiple syscont)" {
  ...
  syscont0=$(docker_run --rm ${CTR_IMG_REPO}/ubuntu-focal-docker:latest tail -f /dev/null)
  syscont1=$(docker_run --rm ${CTR_IMG_REPO}/ubuntu-focal-docker:latest tail -f /dev/null)
...
@test "bind over sysfs submount" {
  ...
  docker exec "$syscont" bash -c "mount -t tmpfs tmpfs $node"
  [ "$status" -eq 0 ]
