...
@test "max_dgram_qlen lookup() operation" {
...
# Read operation.
@test "max_dgram_qlen read() operation" {
  sv_runc run -d --console-socket $CONSOLE_SOCKET syscont
  [ "$status" -eq 0 ]
...
@test "max_dgram_qlen write() operation (lower value)" {
  sv_runc run -d --console-socket $CONSOLE_SOCKET syscont
  [ "$status" -eq 0 ]
...
@test "max_dgram_qlen write() operation (higher value)" {
  sv_runc run -d --console-socket $CONSOLE_SOCKET syscont
  [ "$status" -eq 0 ]
